...
from src.ddx_data_gen.prompt_builder import PromptArgs, get_json_format_examples, build_prompt
from src.ddx_data_gen.prompts import PROMPT_TEMPLATES_MIMIC
...
def build_v1_dict(present_findings):
    all_findings = ["Diffuse Abdominal Pain", "RUQ Abdominal Pain", "Epigastric Abdominal Pain",
                    "LUQ Abdominal Pain", "Flank Abdominal Pain", "Umbilical Abdominal Pain",
                    "RLQ Abdominal Pain", "Hypogastric Abdominal Pain", "LLQ Abdominal Pain",
                    "Fever", "Rigors and Chills", "Nausea or Vomiting", "Jaundice", "Constipation",
                    "Diarrhea", "Weight Loss", "GI Bleeding", "Hypotension", "Guarding",
                    "Rebound Tenderness", "Normal Bowel Sounds", "Hypoactive Bowel Sounds",
                    "Hyperactive Bowel Sounds", "Absent Bowel Sounds",]

    result = {}
    for finding in all_findings:
        if finding in present_findings:
            result[finding] = {
                "value": 1,
                "reasoning": f"This finding ({finding}) is mentioned in the text."
            }
        else:
            result[finding] = {
                "value": -1,
                "reasoning": f"This finding ({finding}) is not mentioned in the text."
            }

    return result


def get_corrupted_output(verifier: int) -> str:
    if verifier == 1:
        return str(build_v1_dict(present_findings=['Diarrhea']))
    elif verifier in (2, 3):
        return convert_mimic_output_to_json('Dumping syndrome', None, verifier)
    elif verifier == 4:
        return convert_mimic_output_to_json(['K91.1', 'K91.89', 'R19.7', 'K52.9', 'K59.1'], None, verifier)


def create_merlin_instructions(patients: pd.DataFrame, corrupted: False) -> pd.DataFrame:
    ...
    for i, row in patients.iterrows():
        ...
        for v in range(1, 5):
            ...
            instructions = pd.concat([instructions, pd.DataFrame({
                ...
                'Thinking': thinking,
                'Output': get_corrupted_output(v) if corrupted else output
