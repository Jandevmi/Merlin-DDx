...
def get_json_schema(pydantic_model, add_reasoning_field: bool):
    """
    Flattens a Pydantic model schema for guided decoding (removes $defs/$ref).
    Optionally wraps the schema with an outer object containing a free-text
    'reasoning' field and the original structured output as 'data'.
    """
    ...
    if not add_reasoning_field:
        return schema

    # --- Add outer reasoning field wrapper ---
    if pydantic_model.__name__ in ("DiagnosesModel", "ICDsModel"):
        schema = {
            "type": "object",
            "title": f"{pydantic_model.__name__}WithReasoning",
            "properties": {
                "reasoning": {
                    "type": "string",
                    "title": "Structured Reasoning Process",
                    "description": (
                        "Step-by-step reasoning, differential diagnosis, and "
                        "consideration of laboratory findings."
                    ),
                },
                "data": schema,  # <- the ICDsModel schema goes here
            },
            "required": ["reasoning", "data"],
        }

    else:
        old_properties = schema.get("properties", {})    # Create new ordered properties with 'reasoning' first
        new_props = OrderedDict()
        new_props["reasoning"] = {"type": "string", "title": "Reasoning"}
        new_props.update(old_properties)
        schema["properties"] = new_props
        schema["required"] = ["reasoning"] + schema["required"]
...
def load_schemes_and_labelspace(patients: pd.DataFrame, prompt_args: PromptArgs, exp_args: ExpArgs):
    ...
    # FixMe: Temporary fix for different chief complaints in abdominal pain results
    unique_complaints = ['abdominal_pain'] if len(unique_complaints) > 7 else unique_complaints
    potential_diagnoses = {}
    for chief_complaint in unique_complaints:
        print(f'Loading diagnoses for chief complaint: {chief_complaint}')
        path = f'{exp_args.data_dir}/medical_schemes/diagnoses/{chief_complaint.replace(" ", "_")}.csv'
